...
# Define the parameters
    ...
    #number_allowed_missing_connections = 5
    number_allowed_missing_connections = 5
...
# Define the constraints 
    ...
    @constraint(model, sum( (sum( (z[i,j] * ( 1 - adjacency(genes[i],genes[j]) )) for j = i+1:amount_of_genes)) for i in 1:amount_of_genes-1 ) <= number_allowed_missing_connections)
