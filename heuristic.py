...
#define the # of missing edges
global nbr_allowed_missing_edges
answer = input('How many allowed missing edges ?\n')

global perc
perc = None

if(answer == 'perc'):
    perc = input('Which percentage ?\n')
    print(f'missing edges allowed is a function of the size of the module')
else:
    nbr_allowed_missing_edges = int(answer)
    print(f'{nbr_allowed_missing_edges} missing edges allowed.')
...
def main(V):
    ...
    for m in range(max_m+1)[2:]:
        ...
        if perc != None:
            nbr_allowed_missing_edges = floor( (int(perc)/100) * factorial(m)/(factorial(m-2)*2))
        ...
        while current_temp > end_temp:
            ...
            if objective(V, m, adj) <= nbr_allowed_missing_edges : # doesn't accept missing connections
                succes = True
                break
